...
export const metadata = { title: 'Бүтэц зохион байгуулалт · Багш ажилтан' };

type Row = Awaited<ReturnType<typeof load>>[number];

async function load() {
  return prisma.staff.findMany({
    where: { positionKey: { startsWith: 'hs-' } },
    orderBy: [{ order: 'asc' }, { position: 'asc' }],
  });
}

export default async function HsAdminStructureListPage() {
  ...
  const columns: Column<Row>[] = [
    ...
    },
    {
      ...
      cell: (s) => (
        <Link
          href={`/high-school/admin/structure/${s.id}/edit`}
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={`Нийт ${items.length} ажилтан · Сургуулийн бүтэц зураг (Танилцуулга хуудас) дээр харагдана.`}
        breadcrumb={[
        ...
        action={
          <Button
            href="/high-school/admin/structure/new"
            variant="primary"
            size="md"
      ...
      <DataTable
        data={items}
        columns={columns}
        empty="Ажилтны мэдээлэл хараахан оруулагдаагүй байна. Шинэ ажилтан нэмнэ үү."
      />
    </>
  );
}
