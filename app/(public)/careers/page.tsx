...
export default async function CareersPage() {
  ...
  return (
    <>
      ...
      <Section background="cream-soft" id="openings">
        ...
        {openings.length === 0 ? (
          ...
          <div className="grid gap-5 md:grid-cols-2">
            {openings.map((o) => (
              /*
                Card is now a Link to the per-opening detail page
                (/careers/<slug>) where the full Шаардлага / Бүрдүүлэх
                материал / Хугацаа / Холбоо info + Анкет татах + Анкет
                бөглөх товчуудтай байна. List page-аас зөвхөн
                заавал хэрэгтэй үндсэн мэдээллийг (title + товч
                description) харуулна — visitor дэлгэрэнгүй уншихыг
                хүсвэл картан дээр дарж detail page-руу шилжинэ.
              */
              <Link
                key={o.slug}
                href={`/careers/${o.slug}`}
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {o.description && (
                      <p className="mt-2 line-clamp-3 text-sm leading-relaxed text-text-body">
                        {o.description}
                      </p>
                    )}
                  </div>
                </div>
                <span className="mt-auto inline-flex items-center gap-1.5 pt-5 text-xs font-bold uppercase tracking-wider text-navy-900 transition-transform group-hover:translate-x-0.5">
                  Дэлгэрэнгүй
      ...
      {/*
        "Бүртгэлийн мэдээлэл" хэсэг list page-ээс хасагдсан — энэхүү
        бүх ерөнхий мэдээлэл одоо /careers/<slug> detail page бүрд
        opening-ийн дэргэд орчихсон тул visitor өөрийн сонгосон
        позицийн контекстэд унших нь илүү ойлгомжтой. SiteContent
        rows нь хэвээр үлдсэн (group `careers` — admin /admin/site-
        content "Нээлттэй ажлын байр" tab-аас засна).
      */}
